...
module TwitterExpander
  ...
  def expand(url, &block)
    uri = uri_for(url)
    begin
      result = open(uri){|socket| socket.read}.to_s.strip
  ...
  def cache
    this = self
    @cache ||= LRU.cache{|url| this.expand(url) }
  end
...
3.times do |i|
  a = Time.now.to_f
  puts TwitterExpander.cache.get('http://is.gd/CsVw')
  puts TwitterExpander.cache.get('http://is.gd/CsVw')
  puts TwitterExpander.cache.get('http://google.com')
  b = Time.now.to_f
  puts "#{ i } : #{ b - a }"
end
